...
echo "🚀 Building all services with Maven + Jib Docker Build..."
...
build_service() {
    ...
    if [ -f "pom.xml" ]; then
        echo "🔨 Running: mvn clean package jib:dockerBuild -Djib.to.image=$image_name:latest"
        ./mvnw clean package jib:dockerBuild -Djib.to.image=$image_name:latest
